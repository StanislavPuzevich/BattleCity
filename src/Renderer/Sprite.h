...
namespace RenderEngine
{
	...
	public:
		...
		uint64_t getFrameDuration(const size_t frameID) const { return m_framesDescriptions[frameID].duration; }
		size_t getFramesCount() const { return m_framesDescriptions.size(); }
		void insertFrames(std::vector<FrameDescription>& framesDescriptions) { m_framesDescriptions = std::move(framesDescriptions); }
